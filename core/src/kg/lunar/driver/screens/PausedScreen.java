...
public class PausedScreen extends Group {
    ...
    public PausedScreen(float w, float h) {
        ...
        title = new Image(LunarDriver.atlas.findRegion("paused"));
        title.setX((w - title.getWidth()) / 2);
        title.setY(h);
        addActor(title);

        resume = new ImageButton(new TextureRegionDrawable(LunarDriver.atlas.findRegion("play_btn")),
                new TextureRegionDrawable(LunarDriver.atlas.findRegion("play_btn_down")
                ));
