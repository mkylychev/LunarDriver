...
public class LevelCompletedScreen extends Group{
    ...
    public LevelCompletedScreen(float w, float h) {
        ...
        title = new Image(LunarDriver.atlas.findRegion("level_completed"));
        title.setX((w-title.getWidth())/2);
        title.setY(h);
        addActor(title);

        done = new ImageButton(
                new TextureRegionDrawable(LunarDriver.atlas.findRegion("done_btn")),
                new TextureRegionDrawable(LunarDriver.atlas.findRegion("done_btn_down")) );
        addActor(done);
