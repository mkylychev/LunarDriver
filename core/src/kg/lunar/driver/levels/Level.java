...
import kg.lunar.driver.controls.CButton;
import kg.lunar.driver.controls.JoyStick;
import kg.lunar.driver.controls.JumpGauge;
import kg.lunar.driver.player.IBody;
import kg.lunar.driver.player.Player;
import kg.lunar.driver.player.UserData;
import kg.lunar.driver.screens.LevelCompletedScreen;
import kg.lunar.driver.screens.LevelFailedScreen;
import kg.lunar.driver.screens.PausedScreen;
...
public class Level extends StageGame {
    ...
    public Level(String directory) {
        ...
        pleaseWait = new Image(LunarDriver.atlas.findRegion("please_wait"));
    ...
    private void setBackGround(String region) {
        clearBackground();
        Image bg = new Image(LunarDriver.atlas.findRegion(region));
    ...
    private void build() {
        ...
        jumpBackBtn = new CButton(
                new Image(LunarDriver.atlas.findRegion("jump1")),
                new Image(LunarDriver.atlas.findRegion("jump1_down")),
                mmToPx(10)
        );
        ...
        jumpForwardBtn = new CButton(
                new Image(LunarDriver.atlas.findRegion("jump2")),
                new Image(LunarDriver.atlas.findRegion("jump2_down")),
                mmToPx(10)
        );
        ...
        pausedScreen.addListener(new MessageListener() {
            ...
            protected void receivedMessage(int message, Actor actor) {
                if (message == PausedScreen.ON_RESUME) {
                    LunarDriver.media.playSound("click.ogg");
                    resumelevel();
    ...
    public void setMusic(String name) {
        musicName = name;
        LunarDriver.media.addMusic(name);
    }
    ...
    public void dispose() {
        if (musicName != null && musicHasLoaded) {
            LunarDriver.media.stopMusic(musicName);
            LunarDriver.media.removeMusic(musicName);
        }
    ...
    private void playMusic() {
        if (musicName != null && musicHasLoaded) {
            LunarDriver.media.playMusic(musicName, true);
        }
    }
        ...
        if (data != null) {
            if (data.name.equals("land") && !player.isHasDestoyed()) {
                if (player.getRotation() < -90 || player.getRotation() > 90) {
                    player.destroy();
                    LunarDriver.media.playSound("crash.ogg");
                    levelFailed();
                } else {
                    player.touchGround();
                }
    ...
    private void levelCompleted() {
        ...
        LunarDriver.media.playSound("level_completed.ogg");
        LunarDriver.media.playMusic("level_win.ogg", false);
    }
